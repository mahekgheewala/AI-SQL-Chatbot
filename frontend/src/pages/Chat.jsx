...
export default function Chat() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const activeDb = localStorage.getItem('active_database');
      if (activeDb && dbs.includes(activeDb)) {
        setSelectedDb(activeDb);
        try {
          await selectDatabase(activeDb);
          const tablesData = await getTables(activeDb);
          setTables(tablesData.tables);
        } catch(err) {
          console.error("Failed to auto-select active database", err);
        }
      }
    };
    init();
      ...
      <header className="bg-white border-b px-6 py-4 flex items-center justify-between shadow-sm z-10 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="text-gray-400 hover:text-violet-600 transition-colors text-lg leading-none"
          >
            📊
          </Link>
        </div>
